...
const mockAddresses: (ShippingInformation & { id: string; name: string; isDefault: boolean; type: 'home' | 'work' | 'other' })[] = [
    {
      id: '1',
      name: 'Casa',
      type: 'home',
...
describe('AddressBook', () => {
  describe('when there are saved addresses', () => {
    ...
    it('should display addresses and select the default one', async () => {
      ...
      const casaCard = screen.getByText('Av. Insurgentes Sur 123').closest('.chakra-card');
      expect(casaCard).toBeInTheDocument();

      // Use a specific selector for the name to avoid ambiguity with the badge
      expect(within(casaCard!).getByText('Casa', { selector: 'p' })).toBeInTheDocument();
    ...
    it('should update selection when a different address is clicked', async () => {
      ...
      const oficinaCard = screen.getByText('Paseo de la Reforma 456').closest('.chakra-card');
  ...
  describe('when there are no saved addresses', () => {
    ...
    it.skip('should display a message that no addresses are saved', () => {
      ...
      expect(screen.queryByText('Av. Insurgentes Sur 123')).not.toBeInTheDocument();
    });
  });
});
