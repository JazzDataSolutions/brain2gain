...
describe("useNotifications", () => {
  beforeEach(() => {
    ...
    mockNotificationService.getConnectionState = vi
      .fn()
  ...
  it("should auto-connect when user is available", () => {
    ...
    expect(mockNotificationService.connect).toHaveBeenCalledWith(
      "user123",
      "admin",
    )
  })
  ...
  it("should disconnect when user becomes unavailable", () => {
    ...
    act(() => {
      const connectedHandler = mockNotificationService.on.mock.calls.find(
  ...
  it("should not add connection/pong messages to notification list", () => {
    ...
    act(() => {
      const notificationHandler = mockNotificationService.on.mock.calls.find(
