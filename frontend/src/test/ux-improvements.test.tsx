...
vi.mock('../stores/cartStore', () => ({
  useCartStore: () => ({
    ...
    clearCart: vi.fn(),
    getTotalPrice: () => mockCartItems.reduce((sum, item) => sum + (item.price * item.quantity), 0),
    getTotalItems: () => mockCartItems.reduce((sum, item) => sum + item.quantity, 0),
    setLoading: vi.fn(),
    setError: vi.fn()
  })
}))
...
describe('UX Improvements', () => {
      ...
      const TestComponent = () => {
        ...
        return (
          <div>
            ...
            <button onClick={toggleCart} data-testid="toggle">Toggle</button>
          </div>
        )
      }
  ...
  describe('InstantSearch Component', () => {
    it('renders search input', () => {
      ...
      const searchInput = screen.getByPlaceholderText(/Buscar proteínas, creatina/)
    ...
    it('shows search results on typing', async () => {
      ...
      global.fetch = vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          products: [
            { id: '1', name: 'Proteína Test', price: 50000, category: 'Proteínas' }
          ]
        })
      })
  ...
  describe('Enhanced ProductCard', () => {
    ...
    it('handles wishlist toggle', async () => {
      ...
      const productCard = screen.getByRole('img', { name: 'Proteína Whey Gold' }).closest('div')
