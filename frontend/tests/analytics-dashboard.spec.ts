...
test.describe('Analytics Dashboard E2E Tests', () => {
  ...
  test('should load and display analytics dashboard', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: 'Analytics Dashboard' })).toBeVisible()
    
    // Check that loading spinner is no longer visible
    await expect(page.locator('text=Loading analytics dashboard')).not.toBeVisible()
  })
  ...
  test('should display KPI summary cards with correct data', async ({ page }) => {
    // Wait for KPI section
    await page.waitForSelector('text=Key Performance Indicators', { timeout: 10000 })
  ...
  test('should have working refresh functionality', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="analytics-dashboard"], text=Analytics Dashboard', { timeout: 15000 })
    ...
    await expect(page.locator('text=Refreshing')).not.toBeVisible({ timeout: 10000 })
  ...
  test('should load data within acceptable time limits', async ({ page }) => {
    ...
    await page.waitForSelector('text=Key Performance Indicators', { timeout: 5000 })
  ...
  test('should maintain data consistency across page refreshes', async ({ page }) => {
    ...
    const initialMRR = await page.locator('text=MRR').locator('..').locator('text=/\\$[0-9,]+/').first().textContent()
    const initialOrders = await page.locator('text=Today\'s Orders').locator('..').locator('text=/[0-9]+/').first().textContent()
    ...
    if (initialMRR) {
      await expect(page.locator('text=MRR').locator('..').locator(`text=${initialMRR}`)).toBeVisible()
    }
    if (initialOrders) {
      await expect(page.locator('text=Today\'s Orders').locator('..').locator(`text=${initialOrders}`)).toBeVisible()
    }
  })
  ...
  test('should display progress bars and visual indicators correctly', async ({ page }) => {
    // Wait for dashboard to load
    await page.waitForSelector('text=Key Performance Indicators', { timeout: 10000 })
    
    // Check for progress bars (they should be present in conversion metrics)
    const progressBars = page.locator('[role="progressbar"], .chakra-progress, div[style*="width:"]')
  ...
  test('should handle periodic real-time updates', async ({ page }) => {
    ...
    const currentTimestamp = await page.locator('text=/Last updated:/').textContent()
