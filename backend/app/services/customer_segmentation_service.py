...
class CustomerSegmentationService:
    ...
    def calculate_rfm_scores(self, analysis_date: datetime = None) -> list[RFMScores]:
        ...
        df["monetary_score"] = pd.qcut(
            df["monetary_value"].rank(method="first"), 5, labels=[1, 2, 3, 4, 5]
        )
    ...
    def _assign_rfm_segment(self, row) -> str:
        ...
        for segment, criteria in self.rfm_segments.items():
            if (
                criteria["R"][0] <= r <= criteria["R"][1]
                and criteria["F"][0] <= f <= criteria["F"][1]
                and criteria["M"][0] <= m <= criteria["M"][1]
            ):
    ...
    def get_segment_analysis(self) -> dict:
        ...
        for _segment, data in segment_counts.items():
            count = data["count"]
        ...
        return {
            ...
            "segments": {
                segment.value: {
                    **data,
                    "profile": (
                        segment_profiles.get(segment.value, {}).__dict__
                        if segment.value in segment_profiles
                        else {}
                    ),
                }
            ...
            "segment_distribution": [
                {
                    ...
                    "color": segment_profiles.get(
                        segment.value,
                        CustomerSegmentProfile(segment, "", [], [], 0, "#6B7280"),
                    ).color,
                }
                for segment, data in segment_counts.items()
            ],
    ...
    def _generate_segment_insights(
        self, segment_counts: dict, total_customers: int
    ) -> list[str]:
        """Generate actionable insights from segmentation analysis"""
        insights = []

        # Check for high-value segments
        champions_pct = segment_counts.get(RFMSegment.CHAMPIONS, {}).get(
            "percentage", 0
        )
        if champions_pct < 5:
            insights.append(
                "Low Champions percentage suggests need for customer retention programs"
            )
        elif champions_pct > 15:
            insights.append(
                "High Champions percentage indicates strong customer loyalty"
            )

        # Check for at-risk customers
        at_risk_pct = segment_counts.get(RFMSegment.AT_RISK, {}).get(
            "percentage", 0
        ) + segment_counts.get(RFMSegment.CANNOT_LOSE, {}).get("percentage", 0)
        if at_risk_pct > 15:
            insights.append(
                f"High percentage ({at_risk_pct:.1f}%) of at-risk customers needs immediate attention"
            )

        # Check for new customer conversion
        new_customers_pct = segment_counts.get(RFMSegment.NEW_CUSTOMERS, {}).get(
            "percentage", 0
        )
        potential_loyalists_pct = segment_counts.get(
            RFMSegment.POTENTIAL_LOYALISTS, {}
        ).get("percentage", 0)
        ...
        hibernating_pct = segment_counts.get(RFMSegment.HIBERNATING, {}).get(
            "percentage", 0
        )
        if hibernating_pct > 10:
            insights.append(
                "Consider cleaning hibernating customers from active marketing lists"
            )

        # Revenue concentration
        champions_revenue = segment_counts.get(RFMSegment.CHAMPIONS, {}).get(
            "total_monetary", 0
        )
        loyal_revenue = segment_counts.get(RFMSegment.LOYAL_CUSTOMERS, {}).get(
            "total_monetary", 0
        )
        total_revenue = sum(
            data.get("total_monetary", 0) for data in segment_counts.values()
        )

        high_value_revenue_pct = (
            ((champions_revenue + loyal_revenue) / total_revenue * 100)
            if total_revenue > 0
            else 0
        )
        ...
        if high_value_revenue_pct > 60:
            insights.append(
                f"High revenue concentration ({high_value_revenue_pct:.1f}%) in top segments - ensure retention"
            )

        return insights

    def get_customers_by_segment(
        self, segment: RFMSegment, limit: int = 100
    ) -> list[dict]:
