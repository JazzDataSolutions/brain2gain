...
class ReportGenerationService:
    ...
    def generate_executive_report(
        ...
        report = {
            ...
            "key_performance_indicators": {
                "revenue": {
                    "monthly_recurring_revenue": float(
                        self.analytics_service.calculate_mrr()
                    ),
                    "annual_recurring_revenue": float(
                        self.analytics_service.calculate_arr()
                    ),
                    "revenue_per_customer": float(
                        current_revenue
                        / max(customer_metrics.get("total_customers", 1), 1)
                    ),
                    "revenue_growth_rate": round(revenue_growth, 2),
                },
                "customer": {
                    "customer_lifetime_value": customer_metrics.get(
                        "avg_customer_lifetime_value", 0
                    ),
                    "customer_acquisition_cost": customer_metrics.get(
                        "estimated_cac", 0
                    ),
                    "payback_period_months": customer_metrics.get(
                        "cac_payback_months", 0
                    ),
                    "net_promoter_score": 8.2,  # Simulated - would come from surveys
                },
                "operational": {
                    "inventory_turnover": financial_summary.get(
            ...
            "customer_segmentation": {
                ...
                "champions_percentage": segmentation_analysis.get("segments", {})
                .get("Champions", {})
                .get("percentage", 0),
                "at_risk_percentage": sum(
                    [
                        segmentation_analysis.get("segments", {})
                        .get("At_Risk", {})
                        .get("percentage", 0),
                        segmentation_analysis.get("segments", {})
                        .get("Cannot_Lose", {})
                ...
                "segment_distribution": segmentation_analysis.get(
                    "segment_distribution", []
                ),
            },
            "cohort_performance": {
                "retention_summary": cohort_analysis.get("summary", {}),
                "avg_1_month_retention": cohort_analysis.get("summary", {})
                .get("avg_retention_by_period", {})
                .get(1, 0),
                "avg_6_month_retention": cohort_analysis.get("summary", {})
                .get("avg_retention_by_period", {})
                .get(6, 0),
            },
    ...
    def generate_operational_report(self, days: int = 7) -> dict:
        ...
        conversion_funnel = self.funnel_service.calculate_conversion_funnel(
            start_date, end_date
        )
        ...
        report = {
            ...
            "real_time_overview": {
                ...
                "conversion_rate_today": realtime_metrics.get(
            ...
            "performance_metrics": {
                ...
                "cart_abandonment_rate": self.analytics_service.get_cart_abandonment_rate(
                    days
                ),
                "average_order_value": float(
                    self.analytics_service.get_average_order_value(start_date, end_date)
                ),
            },
            "inventory_status": {
    ...
    def generate_customer_analytics_report(self, months_back: int = 6) -> dict:
        ...
        # Get cohort analysis
        retention_cohorts = self.cohort_service.calculate_retention_cohorts(
            months_back=months_back
        )
        revenue_cohorts = self.cohort_service.calculate_revenue_cohorts(
            months_back=months_back
        )
        ...
        report = {
            ...
            "customer_segmentation": {
                ...
                "segment_recommendations": {
                    segment.value: self.segmentation_service.get_segment_recommendations(
                        segment
                    )
                    for segment in [
            ...
            "cohort_analysis": {
                ...
                "cohort_insights": self._generate_cohort_insights(
            ...
            "customer_journey": {
                ...
                "conversion_patterns": self._analyze_conversion_patterns(
                    channel_funnels, device_funnels
                ),
            },
            "customer_behavior": {
            ...
            "retention_analysis": {
                ...
                "retention_by_segment": self._get_retention_by_segment(
                    segmentation_analysis
                ),
    ...
    def schedule_automated_reports(self) -> dict:
        ...
        schedules = {
            ...
            "monthly_customer": {
                ...
                "recipients": [
                    "marketing@brain2gain.com",
                    "customer_success@brain2gain.com",
                ],
    ...
    def _generate_executive_insights(
        ...
        if revenue_growth > 10:
            insights.append(
                f"Strong revenue growth of {revenue_growth:.1f}% indicates successful market execution"
            )
        elif revenue_growth < -5:
            insights.append(
                f"Revenue decline of {abs(revenue_growth):.1f}% requires immediate strategic attention"
            )
        ...
        if churn_rate > 10:
            insights.append(
                f"High churn rate ({churn_rate:.1f}%) is impacting growth - focus on retention initiatives"
            )
        ...
        if champions_pct < 10:
            insights.append(
                "Low Champions segment suggests need for customer loyalty programs"
            )

        # Cohort insights
        if cohort_analysis.get("summary", {}):
            avg_retention = (
                cohort_analysis["summary"].get("avg_retention_by_period", {}).get(3, 0)
            )
            if avg_retention < 30:
                insights.append(
                    "Poor 3-month retention indicates product-market fit challenges"
                )

        return insights

    def _generate_action_items(
        self, revenue_growth: float, customer_metrics: dict, segmentation_analysis: dict
    ) -> list[dict]:
    ...
    def _generate_strategic_recommendations(
        self, revenue_growth: float, segmentation_analysis: dict, cohort_analysis: dict
    ) -> list[str]:
        ...
        if revenue_growth < 10:
            recommendations.append(
                "Consider expanding into new product categories or markets"
            )
        ...
        if champions_pct > 15:
            recommendations.append(
                "Leverage Champions segment for referral and advocacy programs"
            )
        ...
        if new_customers_pct > 20:
            recommendations.append(
                "Focus on onboarding optimization to convert new customers to loyalists"
            )
    ...
    def _generate_operational_recommendations(
        self, inventory_metrics: dict, conversion_funnel: dict, financial_summary: dict
    ) -> list[str]:
        ...
        if inventory_metrics.get("low_stock_items", 0) > 5:
            recommendations.append(
                "Implement automated reorder system for inventory management"
            )
    ...
    def _generate_cohort_insights(
        self, retention_cohorts: dict, revenue_cohorts: dict
    ) -> list[str]:
        ...
        if retention_cohorts.get("summary", {}):
            avg_retention_1m = (
                retention_cohorts["summary"]
                .get("avg_retention_by_period", {})
                .get(1, 0)
            )
            if avg_retention_1m > 40:
                insights.append(
                    "Strong 1-month retention indicates good product-market fit"
                )
            elif avg_retention_1m < 25:
                insights.append(
                    "Low 1-month retention suggests onboarding optimization needed"
                )

        return insights

    def _analyze_conversion_patterns(
        self, channel_funnels: dict, device_funnels: dict
    ) -> dict:
        """Analyze conversion patterns across channels and devices."""
        return {
            "best_converting_channel": channel_funnels.get("comparison", {}).get(
    ...
    def _generate_customer_insights(
        self,
        segmentation_analysis: dict,
        retention_cohorts: dict,
        channel_funnels: dict,
    ) -> list[str]:
    ...
    def _generate_marketing_recommendations(
        self, segmentation_analysis: dict, channel_funnels: dict, device_funnels: dict
    ) -> list[str]:
        """Generate marketing-specific recommendations."""
        recommendations = []

        # Channel optimization
        best_channel = channel_funnels.get("comparison", {}).get(
            "best_converting_channel", {}
        )
        if best_channel:
            recommendations.append(
                f"Increase investment in {best_channel.get('channel', '')} - highest converting channel"
            )

        # Device optimization
        mobile_share = device_funnels.get("comparison", {}).get(
            "mobile_traffic_share", 0
        )
        if mobile_share > 60:
            recommendations.append(
                "Prioritize mobile experience optimization - majority of traffic is mobile"
            )
        ...
        if at_risk_pct > 10:
            recommendations.append(
                "Implement automated retention campaigns for at-risk customer segments"
            )
