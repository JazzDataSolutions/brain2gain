...
class TestNotificationService:
    ...
    async def test_notify_order_status_success(self, notification_service):
        ...
        with patch("app.services.notification_service.manager") as mock_manager:
            ...
            result = await notification_service.notify_order_status(
                order_id="12345", status="shipped", customer_id="user123"
            )
    ...
    async def test_notify_new_order_success(self, notification_service):
        ...
        with patch("app.services.notification_service.manager") as mock_manager:
            ...
            result = await notification_service.notify_new_order(
                order_id="order123", customer_name="John Doe", total_amount=156.99
            )
    ...
    def test_get_order_template_mapping(self, notification_service):
        ...
        assert (
            notification_service._get_order_template("confirmed")
            == NotificationTemplate.ORDER_CONFIRMATION
        )
        assert (
            notification_service._get_order_template("shipped")
            == NotificationTemplate.ORDER_SHIPPED
        )
        assert (
            notification_service._get_order_template("delivered")
            == NotificationTemplate.ORDER_DELIVERED
        )
        assert (
            notification_service._get_order_template("cancelled")
            == NotificationTemplate.ORDER_CANCELLED
        )

        # Test default fallback
        assert (
            notification_service._get_order_template("unknown")
            == NotificationTemplate.ORDER_CONFIRMATION
        )
